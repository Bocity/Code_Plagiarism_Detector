...
因此，$P_1$和$P_2$之间的相似性被定义为最佳全局指令嵌入(由最大权重网络流量表示)除以$P_1$中的指令数量。 直观地，$(l_i,r_j)\subseteq E$中的每个流动单元将$f\in P_1$中的指令嵌入到f'j∈P2中的另一个指令中。 只有那些具有大w（li，rj）的边缘，表明fi可以嵌入到f'j中，对最终相似性得分有显着贡献，而较大的σ（P1，P2）表示较小的d（P1，P2）


该算法被实现为命令行工具bincmp，其中ω= 32 | fi |，α= 2，并且β= 1/2。 为了检测一组程序中的代码剽窃，驱动程序将一组程序二进制路径作为参数，调用objdump来提取指令序列，去除系统和公共库，最后管理并行bincmp比较的队列。 实际上，调用bincmp通常需要花费几秒钟的时间。 分析数百个提交（通过成对比较）在多处理器计算机上花费不到一个小时的时间。
